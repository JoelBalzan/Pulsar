...
a = psrchive.Archive_load(intensity_file)
a.remove_baseline()
a.tscrunch()
a.pscrunch()
data = a.get_data()
...
DS = data[0,0,:,:]
pulse_profile = np.mean(DS, axis=0)
...
dt = (a.integration_length()/nbin)*u.s
    ...
    for f in range(len(log_delay_freq_range)):
        ...
        for trail in range(trials):
            try:
                ...
                popt,pcov = curve_fit(gaus,dm_series,data,
                              p0=pars_init,
                              bounds=((0, dm_start, median_width-eps, -np.inf), 
                                      (np.inf, dm_end, median_width+eps, np.inf)))
